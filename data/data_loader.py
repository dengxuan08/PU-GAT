...
class PUNAN_Dataset(data.Dataset):
    def __init__(self, h5_file_path='/home/dx/PycharmProjects/PUGNN/data/PUGAN_poisson_256_poisson_1024.h5',split_dir='train_list.txt',
...
class Test_Dataset(data.Dataset):
    def __init__(self, file_path='/PointCloud', npoints=256):
        super().__init__()

        file_list = os.listdir(file_path)
        self.names = [x.split('.')[0] for x in file_list]
        file_path = os.path.abspath(file_path)
        paths = glob(os.path.join(file_path, '*.xyz'))  # 拿到所有的测试数据路径
        print(len(paths))
        self.data = []
    ...
    def __getitem__(self, index):
        x = self.data[index][:self.npoints,:]
        return x


  
if __name__=="__main__":
    dataset=Test_Dataset(npoints=2048)
