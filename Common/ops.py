...
from tensorflow.python.ops.init_ops import _assert_float_dtype
...
def conv1d(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs = tf.keras.layers.Conv1D(inputs, num_output_channels, kernel_size, stride, padding,
...
def conv2d(inputs,
  ...
  with tf.variable_scope(scope,reuse=reuse) as sc:
      ...
      outputs = tf.keras.layers.Conv2D(inputs,num_output_channels,kernel_size,stride,padding,
                                 kernel_initializer=initializer,
      ...
      assert not (bn and ibn)
      if bn:
          outputs = tf.keras.layers.BatchNormalization(outputs,momentum=bn_decay,training=is_training,renorm=False,fused=True)
          #outputs = tf.keras.layers.batch_norm(outputs,is_training=is_training)
      if ibn:
          outputs = instance_norm(outputs,is_training)
...
def fully_connected(inputs,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        outputs = tf.keras.layers.dense(inputs,num_outputs,
        ...
        if bn:
            outputs = tf.keras.layers.BatchNormalization(outputs, momentum=bn_decay, training=is_training, renorm=False)
